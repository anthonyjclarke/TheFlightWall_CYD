...
ESP32 flight tracker displaying live nearby aircraft on a CYD TFT and embedded web dashboard. OpenSky ADS-B state vectors (OAuth2) → AeroAPI enrichment → FlightWall CDN name lookup → cycling flight card. Current dev version: 1.5.0-dev (last released: 1.4.0). **Canonical version macro: `FW_VERSION_STR` in [`src/config/Version.h`](src/config/Version.h)** — bumping it there propagates to the WebUI title/brand badge and the boot serial banner. Markdown docs (this file, README.md, CHANGELOG.md) stay manually-maintained.
...
**Pinned flight tracking.** `RuntimeConfig::pinnedFlightNumber()` (NVS `pinned_flt`) names a flight always shown as slot 0. `promoteOrFetchPinned()` in `main.cpp` runs each fetch after `g_flights` is resolved: (1) if the flight is already in the in-radius list, set `f.pinned` and `std::rotate` it to front (zero extra API calls); (2) else `AeroAPIFetcher::fetchFlightInfo()` (`/flights/{ident}`) for route/timing/aircraft, then `AeroAPIFetcher::fetchLivePosition()` for live position; `promoteOrFetchPinned()` computes `distance_km`/`bearing_deg` from the configured centre via `GeoUtils`. **Position source gotcha:** `/flights/{ident}` (BaseFlight schema) carries **no** position — `last_position` only exists on the `/flights/search` endpoint (InFlightStatus, airborne flights only), which `fetchLivePosition()` queries with `?query=-idents <IDENT>`. **OpenSky cannot be queried by callsign** either — `/states/all` has no `callsign` filter (only `time`, `icao24`, bbox). Do not reintroduce either dead approach. A not-yet-airborne pinned flight returns no position (shows "Locating…") until `/flights/search` matches it. **Ident format fallback:** AeroAPI's `/flights/search` indexes by ICAO callsign (e.g. `GIA714`), but the user may enter an IATA ident (e.g. `GA714`). `promoteOrFetchPinned()` tries `info.ident` → `info.ident_icao` → `info.ident_iata` in sequence so the correct form is found regardless of what the user typed. "No live position" after all three attempts = AeroAPI coverage gap for that airline/region (e.g. some Indonesian or regional operators). FlightWall enrichment runs, card inserted at front; (3) if AeroAPI has no active record, insert an "Awaiting data..." placeholder (NaN lat/lon) so the card never silently vanishes. Clearing the pin (empty string) strips any `f.pinned` entry immediately in **both** `promoteOrFetchPinned()` and the `takePendingForceFetch()` handler — do not rely on a fetch to drop it, since last-good retention would keep it for cycles. Pinned cards render an amber header strip (`COLOR_ROUTE`) on the TFT; out-of-range pinned flights get a bearing-arrow edge indicator on the map card. `renderKey()` includes the pinned flag so the render guard repaints when a card is promoted.

**Web dashboard data path.** `WebUIServer` receives pointers to `g_flights` and `CYDDisplay`, and `main.cpp` records each completed fetch into a volatile 50-entry activity ring via `recordFetch()`. `GET /api/live` serves the selected display card, **all** `g_flights` (not capped — cap was removed in v0.14.0), activity events, busy/phase state and a `next_fetch_in` countdown; `GET /api/logo` serves cached LittleFS images. The TFT-mirror flight-card render is browser-rendered deliberately (not a framebuffer readback) for low SPI/network cost — but the *map-card* mirror does fetch the cached map JPEG from `/api/mappreview` plus an SVG overlay, since browser-side recreation is not pixel-accurate. `GET /api/screenshot` is the framebuffer-readback path, used on demand only for documentation captures. `GET /api/config` exposes only `opensky_configured` / `aero_configured` boolean flags (plus `pinned_flight`) — never the stored credential strings. `POST /api/config` saves to NVS and **applies live — no auto-reboot** (changed in v1.4.0): it sets `_pendingApply` (any non-pinned change), `_pendingReauth` (OpenSky creds changed), and `_pendingForceFetch` (pinned changed). `main.cpp` polls `takePendingApply()` (applies brightness via `CYDDisplay::setBrightness()` + forces a refresh), `takePendingReauth()` (calls `OpenSkyFetcher::invalidateToken()`), and `takePendingForceFetch()` each loop tick. Reboot is now only via the explicit `POST /api/reboot` route (Reboot Device button) which sets `_pendingReboot`; `main.cpp` reads `shouldReboot()` and triggers `ESP.restart()` after a 400 ms millis delay so the response can flush.

**Slot-tracking parity.** `WebUIServer::onGetLive()` must mirror `CYDDisplay::displayFlights()` exactly when computing the current slot. The map card is only a slot when at least one flight is plottable: `hasMap = anyFlightLocatable(flights)` (free function in `FlightInfo.h` — use the **same** helper in both files), `totalSlots = flights.size() + (hasMap ? 1 : 0)`, `slotIdx = currentFlightIndex % totalSlots`. Emit `screen.kind = "map"` when `hasMap && slotIdx == flights.size()`, otherwise `"flight"`. The conditional `+1` (added v1.4.0) prevents a lone pinned placeholder with no fix from producing a useless empty-map slot that appears to "show the map twice". Do **not** apply `% flights.size()` — that collapses the map slot back to flight #0 and makes it invisible to the dashboard (bug from v1.1.0 until v1.2.0).
