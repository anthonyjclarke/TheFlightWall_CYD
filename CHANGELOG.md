...
### Added
- **Pinned flight tracking** — the user can nominate a specific flight number (IATA e.g. `QF1` or ICAO e.g. `QFA001`) in the WebUI Device Configuration. Each fetch cycle the firmware promotes the pinned flight to slot 0 in the TFT display cycle: if the flight is already within the configured radar radius it is lifted from the normal `g_flights` list; if it is outside the radius an AeroAPI `/flights/{ident}` call retrieves its route/timing and a second `/flights/search?query=-idents …` call (`AeroAPIFetcher::fetchLivePosition()`) retrieves live position — `last_position` (lat/lon/altitude/groundspeed/heading), with `promoteOrFetchPinned()` computing distance/bearing from the configured centre via `GeoUtils`. (The `/flights/{ident}` BaseFlight schema carries no position, and OpenSky's `/states/all` has no `callsign` filter, so the airborne-only `/flights/search` endpoint is the out-of-radius position source; a not-yet-airborne pinned flight shows "Locating…" until it matches.) Position search tries `ident` → `ident_icao` → `ident_iata` in sequence — AeroAPI's `/flights/search` indexes by ICAO callsign, so a user-entered IATA ident (e.g. `GA714`) will fall through to the ICAO form (`GIA714`) automatically. FlightWall logo and airline-name enrichment run as normal (logos are LittleFS-cached after the first cycle). If AeroAPI returns no active record a placeholder card ("Awaiting data...") is shown at slot 0 so the TFT always reflects that a pinned flight is configured. Clicking "Update" (or pressing Enter) in the WebUI immediately runs `promoteOrFetchPinned()` — full route, position and enrichment are fetched on the spot without waiting for the normal fetch interval. The pinned card TFT header is rendered with an amber (`COLOR_ROUTE`) strip background and dark counter text for instant visual identification. The WebUI flight JSON includes `"pinned": true` on that entry and shows "Saved — fetching now…" feedback. Config key `"pinned_flt"` in NVS; no reboot required.
  - **WebUI pinned indicators** — the pinned flight is now visually distinct from enriched/ADS-B cards: an amber `PINNED` badge (in addition to the `ENRICHED`/`ADS-B` tag), an amber card border/glow, an amber header strip on the TFT-mirror card matching the CYD, and a clickable `📍 Show Location` link (Google Maps URL embedded, no coordinates exposed in text) plus dedicated `Latitude`/`Longitude` facts. When the pinned flight has no live position yet the card shows `📍 Locating…`.
  - **Immediate pin removal** — clearing the pinned flight in the WebUI now strips the pinned card from `g_flights` instantly (on both the force-fetch path and inside `promoteOrFetchPinned`) and forces a refresh, fixing the lag where a stale pinned card lingered for several cycles via last-good retention.

### Changed
- **Map card skipped when nothing is plottable** — the radar map slot is now only part of the display cycle when at least one flight carries a position (new `anyFlightLocatable()` helper in `FlightInfo.h`, used identically by `CYDDisplay::displayFlights()` and `WebUIServer::onGetLive()` to preserve slot-tracking parity). Previously a lone pinned placeholder with no fix produced a two-slot cycle (placeholder ↔ empty map), making the map appear to "show twice"; now the placeholder card shows on its own until a position is acquired.
- **Settings apply live — no reboot required.** `POST /api/config` no longer schedules a reboot. All values apply within seconds: brightness is written to the backlight immediately (new `CYDDisplay::setBrightness()`, also applied at boot from NVS), OpenSky credential changes drop the cached OAuth token (new `OpenSkyFetcher::invalidateToken()`) so new creds take effect on the next fetch, and location / radius / timing / label-colour changes trigger an immediate refresh. The WebUI "Save & Reboot" button is now "Save" ("Settings applied — no reboot needed."), with a separate **Reboot Device** button (`POST /api/reboot`) retained for WiFi reset or recovery. Main loop polls `takePendingApply()` / `takePendingReauth()` alongside the existing reboot/force-fetch flags.
