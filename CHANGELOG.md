...
### Added
- **Pinned flight tracking** — the user can nominate a specific flight number (IATA e.g. `QF1` or ICAO e.g. `QFA001`) in the WebUI Device Configuration. Each fetch cycle the firmware promotes the pinned flight to slot 0 in the TFT display cycle: if the flight is already within the configured radar radius it is lifted from the normal `g_flights` list; if it is outside the radius an extra AeroAPI call retrieves its route and a separate OpenSky callsign query (`GET /api/states/all?callsign=…`) provides live position, altitude, and speed. FlightWall logo and airline-name enrichment run as normal (logos are LittleFS-cached after the first cycle). If AeroAPI returns no active record a placeholder card ("Awaiting data...") is shown at slot 0 so the TFT always reflects that a pinned flight is configured. Clicking "Update" in the WebUI immediately inserts the placeholder on the TFT and triggers a force-fetch cycle (`g_lastFetchMs = 0`) so data arrives within one fetch interval; the status bar shows "Refreshing…" during user-triggered fetches. The pinned card TFT header is rendered with an amber (`COLOR_ROUTE`) strip background and dark counter text for instant visual identification, replacing the earlier small-dot indicator. The WebUI flight JSON includes `"pinned": true` on that entry and shows "Saved — fetching now…" feedback. Config key `"pinned_flt"` in NVS; no reboot required.
  - **WebUI pinned indicators** — the pinned flight is now visually distinct from enriched/ADS-B cards: an amber `PINNED` badge (in addition to the `ENRICHED`/`ADS-B` tag), an amber card border/glow, an amber header strip on the TFT-mirror card matching the CYD, and a clickable Google Maps location link (`📍 lat, lon · N km — open map`) plus dedicated `Latitude`/`Longitude` facts. When the pinned flight has no live position yet the card shows `📍 Locating…`.
